...
foreach (new DirectoryIterator($postsDirectory) as $fileInfo) {
    ...
    echo "<a href='editpost_".$fileInfo->getBasename('.markdown')."'>".$fileInfo->getBasename('.markdown') . "</a><br>\n";
}
echo '</div>
...
echo "<form class='form form-horizontal' method='post' action='edit_push'>";
echo "<input type='hidden' name='filetopush' value='".$current."'>";
echo '<div class="form-group"><div class="col-12 col-sm-12 col-md-12 col-lg-12"><button class="btn btn-success btn-large pull-right" type="submit"  name="edit" value="Edit" />Submit Edit</button><a class="btn btn-info btn-large pull-left" href="/edit_index" />Back to Posts</a><a class="btn btn-warning btn-large pull-left" href="/action_create" />New Post</a></div></div>';
